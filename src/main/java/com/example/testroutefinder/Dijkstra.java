...
//We're not at the goal node yet, so...
    ...
    public static <T> int findPathCost(LinkedList<T> path, GraphNodeAL2<T> startNode){ //Takes a path list and the start node
        if (path==null) return 0; //If the path is null, return 0

        var curNode = startNode; //Set the current node to the start node
        var cost = 0; //Set the cost to 0
        while (!path.isEmpty()) { //While the path is not empty...
            var nextVal = path.removeFirst(); //Remove the first value in the path list
            for (var edge : curNode.adjList) { //For each edge from the current node...
                if (edge.destNode.data.equals(nextVal)) { //If the edge leads to the next value in the path list...
                    cost += edge.cost; //Add the cost of the edge to the total cost
                    curNode = (GraphNodeAL2<T>) edge.destNode; //Set the current node to the destination node
                    break;// Break out of the for loop to continue
                }
            }
        }
